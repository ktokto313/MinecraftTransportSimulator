...
public class WrapperWorld extends AWrapperWorld {
    ...
    public void spawnExplosion(Point3D location, double strength, boolean flames) {
        Explosion explosion = new ExplosionTemp(this.world, null, location.x, location.y, location.z,(float) strength, flames, ConfigSystem.settings.general.blockBreakage.value);
...
class ExplosionTemp extends Explosion {
    private float size;
    private World world;
    private double x;
    private double y;
    ...
    public ExplosionTemp(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain)
    {
        ...
        this.y = y;
        this.z = z;
        this.exploder = super.getExplosivePlacedBy();
    }
        ...
        for (int j = 0; j < i; ++j)
        {
            for (int k = 0; k < i; ++k)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    {
                        double d0 = (double)((float)j / (float)i * 2.0F - 1.0F);
                        double d1 = (double)((float)k / (float)i * 2.0F - 1.0F);
                        double d2 = (double)((float)l / (float)i * 2.0F - 1.0F);
